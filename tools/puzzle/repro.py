...
def derived_files() -> list[Path]:
    """Every derived artifact the pipeline regenerates -- **excluding this run's own report**.

    `recon/derived/reproduction.json` is written by :func:`main`, *after* the cold comparison has
    already run. A cold run that deleted it could therefore never regenerate it during the pipeline and
    would always report it as a difference -- which is not a hypothetical: measured 2026-09-13 (F6), the
    first cold run after the report itself became a tracked file printed

        32 of 33 regenerated byte-identically
        DIFFERS: recon/derived/reproduction.json (not regenerated)

    making `check_stepE4`'s `differences == []` **unsatisfiable by any real run**. The gate had passed
    for as long as it was reading the stale report from before the file was tracked -- that report
    records `tracked_before: 31, differences: []` and lists `reproduction.json` as a *new untracked*
    file, i.e. it predates its own subject being tracked. Excluding the report is the fix; the gate now
    asserts the exclusion, and the report records it (`cold_check.report_excluded`) so the claim is
    checkable from the artifact rather than from this docstring.
    """
    out: list[Path] = []
    for g in DERIVED_GLOBS:
        out.extend(sorted(p for p in ROOT.glob(g) if p.resolve() != OUT.resolve()))
    return out
...
def run_pipeline(cold: bool, out_path: Path) -> tuple[list[dict], dict, int]:
    ...
    if cold:
        ...
        print(f'  ({OUT.relative_to(ROOT).as_posix()} is written after this comparison and is '
              f'therefore not in that set -- see derived_files())\n')
    ...
    if cold and rc == 0:
        ...
        check = {'tracked_before': len(before), 'regenerated_identically': same,
                 'report_excluded': str(OUT.relative_to(ROOT).as_posix()),
