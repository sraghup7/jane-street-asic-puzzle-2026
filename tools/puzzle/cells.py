...
//
// Provenance: the models come from A3's pin names and B5's directions. The hashes below are of those
// two *artifacts as committed* -- build/puzzle.v carries the hash of the GDS the stages read:
//   recon/derived/pin_names.json     sha256 {a3sha}
//   recon/derived/netlist_check.json sha256 {b5sha}
//
...
def stage_cells() -> int:
    ...
    # F5 (2026-09-13): these two lines used to read each artifact's `['source']['sha256']`, which is
    # the hash of the *GDS that artifact read* -- so the header printed puzzle.gds's hash twice and
    # labelled both as artifact hashes, which made the stamp useless for detecting a stale input.
    # The stamp must be the hash of the artifact itself.
    def _sha(p: Path) -> str:
        """The sha256 of one artifact, for the `cells.v` provenance stamp."""
        return hashlib.sha256(p.read_bytes()).hexdigest()

    a3sha = _sha(ROOT / 'recon' / 'derived' / 'pin_names.json')
    b5sha = _sha(ROOT / 'recon' / 'derived' / 'netlist_check.json')
