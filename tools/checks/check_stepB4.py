...
def main() -> int:
    ...
    # ---- 6. the unassigned pins are exactly A5's one class -------------------------
    # The antenna diode's non-VPB supply pins (VGND/VNB/VPWR) used to be gap class 2: reading
    # `cell.polygons` alone dropped the met1 path that draws their rail, so they looked
    # unrouted (review R2). With paths included they are routed and assigned to a net like any
    # other supply pin -- `diode_supplies` below counts how many exist (30, unchanged), not how
    # many are unassigned (0, now).
    vpb = sum(1 for i, pins in sup_pins.items() for p in pins if p == 'VPB')
    diode_supplies = sum(1 for i, pl in enumerate(placements)
                         for p in sup_pins[pl['id']]
                         if p != 'VPB' and 'diode_2' in pl['master'].replace(STD, ''))
    check('the VPB well ties are the whole of the one gap class', vpb, 942)
    check('the antenna diode has 30 non-VPB supply pins, all routed via a met1 path',
          diode_supplies, 30)
    check('unassigned supply pins re-derived', t['unassigned_supply'], vpb)
    check('gap class 1 in the artifact', d['gap_classes']['vpb_well_tie_no_routeable_geometry'],
          vpb)
    check('no antenna-diode gap class remains in the artifact',
          d['gap_classes']['antenna_diode_supply'], 0)
    check('nothing unassigned outside that one class',
          t['unassigned_supply_unexplained'], 0)
    check('every unassigned entry names the one class',
          sorted({u['gap_class'] for u in d['unassigned']}),
          ['vpb_well_tie_no_routeable_geometry'])
    ...
    # 10 more of each than before paths were read (review R2): the antenna diode's supplies
    # now reach these nets instead of being stranded.
    check('the largest net carries VGND and VNB only',
          d['nets'][0]['pin_names'], {'VGND': 1618, 'VNB': 942})
    check('the VPWR net carries VPWR only', d['nets'][1]['pin_names'], {'VPWR': 1618})
